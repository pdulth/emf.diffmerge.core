pipeline {
  ...
  environment {
    //FROM_GERRIT = "${GERRIT_REFSPEC}".contains("refs/");
    //DM_BRANCH = "${FROM_GERRIT}".replace("true", "${GERRIT_BRANCH}").replace("false","${GERRIT_REFSPEC}");
    //BUILD_KEY = "${FROM_GERRIT}".replace("true", "${GERRIT_BRANCH}-${GERRIT_REFSPEC}-${BUILD_ID}").replace("false","${GERRIT_REFSPEC}-${BUILD_ID}").replaceFirst(/^v/, "").replaceAll('/','-');
    ...
    //PROXY_KEY = "${GERRIT_REFSPEC}".replaceFirst(/^v/, "").replaceAll('/','-');
    //PROXY = "/home/data/httpd/download.eclipse.org/diffmerge/nightly/${PROXY_KEY}/emf-diffmerge-site"
  }
  stages {
    stage('Package') {
      steps {
